...
class DataGetter:
    ...
    def _transform_market_data(self, raw_df: pd.DataFrame, period: str) -> pd.DataFrame:
        ...
        raw_df['datetime'] = raw_df.index
        raw_df.drop(columns=['time'], inplace=True)
        # 删除多余的列，只保留必需字段
        keep_cols = DataFields.MARKET_DATA_REQUIRED
        df = raw_df[keep_cols]
        # 数据类型转换
    ...
    def get_real_time_data(self, codes: Union[str, List[str]]) -> Union[Dict[str, pd.DataFrame], pd.DataFrame]:
        ...
        if real_data is not None:
            for trans_code, tick_data in real_data.items():
                ...
                standardized_data['datetime'] = datetime.datetime.strptime(tick_data.get('timetag', None), '%Y%m%d %H:%M:%S')
